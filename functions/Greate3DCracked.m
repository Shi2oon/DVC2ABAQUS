function Greate3DCracked(folder,Len,A,offset)
...
% builidng a rectangle based on the x and y dim
fprintf(fileID,'mdb.models["Model-1"].sketches["__profile__"].rectangle(point1=(%f, %f), \n',Len(1,1),Len(2,1));
fprintf(fileID,'    point2=(%f, %f))\n',Len(1,2),Len(2,2));
fprintf(fileID,'mdb.models["Model-1"].Part(dimensionality=THREE_D, name="Part-1", type=\n');
fprintf(fileID,'    DEFORMABLE_BODY)\n');
% extrusion in 3rd dim using z
...
fprintf(fileID,'mdb.models["Model-1"].ConstrainedSketch(gridSpacing=0.5, name="__profile__", \n');
fprintf(fileID,'    sheetSize=15, transform=\n');
fprintf(fileID,'    mdb.models["Model-1"].parts["Part-1"].MakeSketchTransform(\n');
fprintf(fileID,'    sketchPlane=mdb.models["Model-1"].parts["Part-1"].faces[4], \n');
fprintf(fileID,'    sketchPlaneSide=SIDE1, \n');
fprintf(fileID,'    sketchUpEdge=mdb.models["Model-1"].parts["Part-1"].edges[0], \n');
% defining the centre of the sketch based on x/2 y/2 z
fprintf(fileID,'    sketchOrientation=RIGHT, origin=(%f, %f, %f)))\n',Len(1,2)/2,Len(2,2)/2,Len(3,1));
fprintf(fileID,'mdb.models["Model-1"].parts["Part-1"].projectReferencesOntoSketch(filter=\n');
fprintf(fileID,'    COPLANAR_EDGES, sketch=mdb.models["Model-1"].sketches["__profile__"])\n');

% if Len(3,2) < 0
%         fprintf(fileID,'mdb.models["Model-1"].sketches["__profile__"].Line(point1=(%f, 0.0), point2=(\n',...
%         -Len(1,2)/2,Len(3,2)-Len(2,2)/2);
%     fprintf(fileID,'    %f, %f))\n',Len(1,2)/2-Len(3,2),Len(3,2)-Len(2,2)/2);
% else
    fprintf(fileID,'mdb.models["Model-1"].sketches["__profile__"].Line(point1=(%f, %f), point2=(\n',...
        Len(4,1)-Len(1,2)/2,Len(3,2)-Len(2,2)/2);
    fprintf(fileID,'    %f, %f))\n',Len(4,2)-Len(1,2)/2,Len(3,2)-Len(2,2)/2);
% end
...
fprintf(fileID,'    mask=("[#8 ]", ), )), crackTip=Region(\n');
fprintf(fileID,'    edges=mdb.models["Model-1"].rootAssembly.instances["Part-1-1"].edges.getSequenceFromMask(\n');
fprintf(fileID,'    mask=("[#8 ]", ), )), extensionDirectionMethod=Q_VECTORS, midNodePosition=\n');
fprintf(fileID,'    0.5, name="Crack-1", qVectors=((\n');
fprintf(fileID,'    mdb.models["Model-1"].rootAssembly.instances["Part-1-1"].vertices[2], \n');
fprintf(fileID,'    mdb.models["Model-1"].rootAssembly.instances["Part-1-1"].InterestingPoint(\n');
fprintf(fileID,'    mdb.models["Model-1"].rootAssembly.instances["Part-1-1"].edges[2], \n');
fprintf(fileID,'    MIDDLE)), ), symmetric=OFF)\n');
fprintf(fileID,'mdb.models["Model-1"].HistoryOutputRequest(contourIntegral="Crack-1", \n');
fprintf(fileID,'    createStepName="Step-1", frequency=1, name="H-Output-2", numberOfContours=\n');
fprintf(fileID,'    60, rebar=EXCLUDE, sectionPoints=DEFAULT)\n');
fprintf(fileID,'mdb.models["Model-1"].HistoryOutputRequest(contourIntegral="Crack-1", \n');
fprintf(fileID,'    contourType=K_FACTORS, createStepName="Step-1", name="H-Output-3", \n');
fprintf(fileID,'    numberOfContours=60, rebar=EXCLUDE, sectionPoints=DEFAULT)\n');
fprintf(fileID,'mdb.models["Model-1"].HistoryOutputRequest(contourIntegral="Crack-1",\n');
